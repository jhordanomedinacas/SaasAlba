...
import { MessageSquare, Clock, TrendingUp, Plus, Tag, BarChart2 } from 'lucide-react';
import NewSessionModal from '../components/NewSessionModal';
import { RatingModal } from '../components/RatingModal';
import { useSessions, SessionStatus, ChatSession } from '../context/SessionsContext';
...
export default function HistoryChats() {
  ...
  return (
    <>
      ...
      {viewingSession?.evaluation && (
        <RatingModal
          evaluation={viewingSession.evaluation}
          profileName={viewingSession.scenario}
            ...
            {sessions.map((session) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 md:gap-3 flex-shrink-0">
                  ...
                  {/* Botón ver evaluación — solo si tiene evaluación guardada */}
                  {session.evaluation && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                      ...
                      className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium border border-slate-200 text-slate-600 rounded-lg hover:border-[#0F2C32]/40 hover:text-[#0F2C32] hover:bg-[#0F2C32]/5 transition-colors"
